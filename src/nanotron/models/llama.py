    ...
    def forward_with_hidden_states(
        ...
        if isinstance(input_ids, torch.Tensor):
            ...
            if self.parallel_context.sp_pg.size() > 1:
                ...
                # logger.log(logging.INFO, f"input_ids: {input_ids}")
                # logger.log(logging.INFO, f"input_mask: {input_mask}")
                # logger.log(logging.INFO, f"position_ids: {position_ids}")
        else:
...
class Loss(nn.Module):
    ...
    def forward(
        ...
        if isinstance(label_ids, torch.Tensor) and self.sp_pg.size() > 1:
            ...
            # logger.log(logging.INFO, f"label_ids: {label_ids}")
            # logger.log(logging.INFO, f"label_mask: {label_mask}")
        loss = sharded_cross_entropy(
            sharded_logits, label_ids.transpose(0, 1).contiguous(), group=self.tp_pg, dtype=torch.float
        ).transpose(0, 1)
        #if torch.isnan(loss.detach()).any().item():
        #    logger.log(logging.INFO, f"label_ids: {label_ids}")
        # logger.log(logging.INFO, f"loss: {loss}")
