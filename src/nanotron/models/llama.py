...
class MLA(nn.Module):
    ...
    def forward(
        ...
        #     lower_triangular_mask = torch.tril(torch.ones(seq_len, seq_len, device=sequence_mask.device))  # lower triangle
        ...
        # assume causal pretraining right now
        final_mask = None

        output = F.scaled_dot_product_attention(q, k, v, attn_mask=final_mask, is_causal=True)
        output = (
