        ...
        for group in self.param_groups:
            for p in group["params"]:
                if p.requires_grad:
                    ...
                    state["delta"] = _full_tensor(torch.zeros_like(p))
    ...
    def step(self, closure: Callable | None = None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                state["delta"].add_(_full_tensor(p.grad), alpha=lr)
                ...
                # Set grad to values
                p.grad.copy_(new_grad if not isinstance(p.grad, DTensor) else DTensor.from_local(new_grad, p.grad.device_mesh, p.grad.placements))
